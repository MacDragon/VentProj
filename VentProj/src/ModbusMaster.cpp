...
void ModbusMaster::beginTransmission(uint16_t const u16Address) {
...
/* 	Retrieve data from response buffer.
	...
	@ingroup buffer
 */
uint16_t ModbusMaster::getResponseBuffer(uint8_t const u8Index) const {
	return (u8Index < ku8MaxBufferSize) ? _u16ResponseBuffer[u8Index] : 0xFFFF;
}
...
/* 	Place data in transmit buffer.
	...
	@ingroup buffer
 */
uint8_t ModbusMaster::setTransmitBuffer(uint8_t const u8Index, uint16_t const u16Value) {
...
/* TODO: eliminate this function in favor of using existing MB request functions */
uint8_t ModbusMaster::requestFrom(uint16_t const address, uint16_t quantity) {
	uint8_t read = 1; // krl: added this to prevent warning. This method is not called anywhere...
...
void ModbusMaster::sendBit(bool const data) {
...
uint16_t ModbusMaster::receive(void) {
	return (_u8ResponseBufferIndex < _u8ResponseBufferLength) ? _u16ResponseBuffer[_u8ResponseBufferIndex++] : 0xFFFF;
}
...
@ingroup discrete
 */
uint8_t ModbusMaster::readCoils(uint16_t const u16ReadAddress, uint16_t const u16BitQty) {
...
@ingroup discrete
 */
uint8_t ModbusMaster::readDiscreteInputs(uint16_t const u16ReadAddress, uint16_t const u16BitQty) {
	_u16ReadAddress = u16ReadAddress;
	_u16ReadQty = u16BitQty;
...
@return 0 on success; exception number on failure
 */
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t const u8MBFunction) {
	uint8_t 	u8ModbusADU[256];
	uint8_t 	u8ModbusADUSize = 0, i, u8Qty;
	...
	case ku8MBWriteMultipleCoils:
		u8ModbusADU[u8ModbusADUSize++] = highByte(_u16WriteQty);
		u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16WriteQty);
		u8ModbusADU[u8ModbusADUSize++] = u8Qty = (_u16WriteQty % 8) ? ((_u16WriteQty >> 3) + 1) : (_u16WriteQty >> 3);

		for (i = 0; i < u8Qty; ++i)
			u8ModbusADU[u8ModbusADUSize++] = (i % 2) ? highByte(_u16TransmitBuffer[i >> 1]) : lowByte(_u16TransmitBuffer[i >> 1]);
		break;
	...
	if (!u8MBStatus) {
		...
		case ku8MBReadDiscreteInputs:
			...
			if (u8ModbusADU[2] % 2) {
				if (i < ku8MaxBufferSize)
					_u16ResponseBuffer[i] = (uint16_t) u8ModbusADU[2 * i + 3];
