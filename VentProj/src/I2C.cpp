...
device{ LPC_I2C0 } {
	...
	Chip_I2C_SetClockDiv(device, clock_divider);
...
bool I2C::transaction(uint8_t const devAddr, uint8_t const * const txBuffPtr, uint16_t const txSize, uint8_t* const rxBuffPtr, uint16_t const rxSize) const {
	...
	return i2cmXferRec.status == I2CM_STATUS_OK;
}
 ...
 */
uint32_t I2C::I2CM_XferBlocking(LPC_I2C_T* const pI2C, I2CM_XFER_T* const xfer) {
	uint32_t ret = 0;
	/* start transfer */
